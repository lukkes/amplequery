  ...
  insertText: {
    "Empty": async function(app) {
      try {
        ...
        const result = await app.prompt("Looking for empty notes. Should I mark them for deletion?",
            {
              inputs: [
                {
                  "type": "radio",
        ...
        for await (const noteHandle of app.filterNotes({group: "^vault"})) {
          ...
          try {
            ...
            // Ignore notes that have hidden tasks, as they cannot be empty
            if (noteContent.includes("# Hidden tasks")) continue;

            // Ignore content representing completed tasks
            noteContent = noteContent.slice(0, noteContent.indexOf("# Completed tasks<!-- {\"omit\":true} -->"));

            // If remaining content is empty, we have a match
            if (noteContent.trim() === "" || !noteContent.match(/[^\s\\]/mg)) {
              ...
              if (result) await app.addNoteTag({"uuid": noteHandle.uuid}, "-amplequery-to-be-deleted");
            }
          } catch (err) {
            console.log("Caught an error:", err);
            console.log("Error type:", err.constructor.name); // Log the error type
    ...
    "Images": async function(app) {
      ...
      const result = await app.prompt("Looking for notes with images. Should I filter to a certain tag only?",
          {
            inputs: [
              {
                "type": "tags"
    ...
    "Reference": async function(app) {
      try {
        let note = await app.prompt("Choose a note you want to get all the references for",
    ...
    "Tag Reference": async function(app) {
      try {
        let tag = await app.prompt("Enter the name of the tag you want to find all tag references for",
        ...
        for (let note of tagNotes) {
          ...
          for (let ref of refs) {
            ...
            result  = result.concat(await Promise.all(taggedTasks.map(async (task) => {
              let note = await app.findNote({uuid: task.noteUUID});
              return {
                note: note,
                task: task,
              };
